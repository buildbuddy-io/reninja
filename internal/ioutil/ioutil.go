...
func (c *CustomCommitWriteCloser) Commit() error {
	if c.committed {
		return statuserr.FailedPreconditionError("CommitWriteCloser already committed, cannot commit again")
	}
	...
	if committer, ok := c.w.(Committer); ok {
...
// NewCustomCommitWriteCloser wraps an io.Writer/CommittedWriteCloser
...
// may allocate a lot.
func NewDoubleBufferWriter(ctx context.Context, w CommittedWriteCloser, bufPool *bytebufferpool.VariableSizePool, minimumBufSize, maximumBufSize int) *DoubleBufferWriter {
