// Code generated by re2go 4.3 on Fri Sep  5 23:00:42 2025, DO NOT EDIT.
//
//go:generate re2go $INPUT -o $OUTPUT -i --api generic
package lexer
...
func (l *Lexer) Error(message string) error {
	...
	col := l.lastToken - lineStart + 1

	err := fmt.Sprintf("%s:%d:%d: %s", l.filename, line, col, message)

	// Add some context to the message
	const truncateColumn = 72
	if col > 0 && col < truncateColumn {
		lineEnd := lineStart
		for lineEnd < len(l.input) && l.input[lineEnd] != '\n' && lineEnd-lineStart < truncateColumn {
			lineEnd++
		}
		truncated := lineEnd-lineStart >= truncateColumn

		err += "\n" + l.input[lineStart:lineEnd]
		if truncated {
			err += "..."
		}
		err += "\n" + strings.Repeat(" ", col-1) + "^ near here"
	}
