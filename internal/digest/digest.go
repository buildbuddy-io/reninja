...
func CASResourceNameFromProto(in *rspb.ResourceName) (*CASResourceName, error) {
	if in.GetCacheType() != rspb.CacheType_CAS {
		return nil, statuserr.FailedPreconditionErrorf("ResourceName is not a CAS resource name: %s", in)
	}
...
func ACResourceNameFromProto(in *rspb.ResourceName) (*ACResourceName, error) {
	if in.GetCacheType() != rspb.CacheType_AC {
		return nil, statuserr.FailedPreconditionErrorf("ResourceName is not an AC resource name: %s", in)
	}
...
func (r *ResourceName) CheckCAS() (*CASResourceName, error) {
	if r.rn.GetCacheType() != rspb.CacheType_CAS {
		return nil, statuserr.FailedPreconditionErrorf("ResourceName is not a CAS resource name: %s", r.rn)
	}
...
func (r *ResourceName) CheckAC() (*ACResourceName, error) {
	if r.rn.GetCacheType() != rspb.CacheType_AC {
		return nil, statuserr.FailedPreconditionErrorf("ResourceName is not an AC resource name: %s", r.rn)
	}
...
func (r *ResourceName) Validate() error {
	d := r.rn.GetDigest()
	if d == nil {
		return statuserr.InvalidArgumentError("Invalid (nil) Digest")
	}
	if d.GetSizeBytes() < 0 {
		return statuserr.InvalidArgumentErrorf("Invalid (negative) digest size")
	}
	if d.GetSizeBytes() == int64(0) {
		if r.IsEmpty() {
			return nil
		}
		return statuserr.InvalidArgumentError("Invalid (zero-length) SHA256 hash")
	}
	hash := d.GetHash()
	if expected := hashLength(r.GetDigestFunction()); len(hash) != expected {
		return statuserr.InvalidArgumentErrorf("Invalid length hash. Expected len %v for %s function. Got %v",
			expected, r.GetDigestFunction().String(), len(hash))
	}
	if !isLowerHex(hash) {
		return statuserr.InvalidArgumentError("Hash isn't all lower case hex characters.")
	}
	return nil
}
...
func HashForDigestType(digestType repb.DigestFunction_Value) (hash.Hash, error) {
	...
	case repb.DigestFunction_UNKNOWN:
		...
		return sha256.New(), nil
	default:
		return nil, statuserr.UnimplementedErrorf("No support for digest type: %s", digestType)
	}
}
...
func parseResourceName(resourceName string, cacheType rspb.CacheType, resourceType resourceNameType) (*ResourceName, error) {
	...
	if len(pieces) < 3 || (cacheType == rspb.CacheType_AC && len(pieces) < 4) || (resourceType == uploadResourceName && len(pieces) < 5) {
		return nil, statuserr.InvalidArgumentErrorf("Unparseable resource name, not enough pieces: %s", resourceName)
	}
	...
	if err != nil {
		return nil, statuserr.InvalidArgumentErrorf("Unparseable resource name, invalid size: %s", resourceName)
	}
	if sizeBytes < 0 {
		return nil, statuserr.InvalidArgumentErrorf("Unparseable resource name, negative size: %s", resourceName)
	}
	...
	if inferredDigestFunction == repb.DigestFunction_UNKNOWN {
		return nil, statuserr.InvalidArgumentErrorf("Unparseable resource name, invalid hash (wrong length): %s", resourceName)
	}
	if !isLowerHex(hash) {
		return nil, statuserr.InvalidArgumentErrorf("Unparseable resource name, invalid hash (bad character(s)): %s", resourceName)
	}
	...
	if cacheType == rspb.CacheType_AC {
		if piece != "ac" {
			return nil, statuserr.InvalidArgumentErrorf("Unparseable Action Cache resource name, missing 'ac' blob type: %s", resourceName)
		}
		pieceIdx--
		if pieceIdx >= 0 {
			piece = pieces[pieceIdx]
	...
	if piece == "zstd" {
		...
		return nil, statuserr.InvalidArgumentErrorf("Unparseable resource name, invalid compressed blob type: %s", resourceName)
	}
	...
	if resourceType == uploadResourceName {
		if guuid.Validate(piece) != nil {
			return nil, statuserr.InvalidArgumentErrorf("Unparseable upload resource name, invalid UUID: %s", resourceName)
		}
		pieceIdx--
		if pieceIdx < 0 {
			return nil, statuserr.InvalidArgumentErrorf("Unparseable upload resource name name, not enough pieces: %s", resourceName)
		}
		piece = pieces[pieceIdx]
		if piece != "uploads" {
			return nil, statuserr.InvalidArgumentErrorf("Unparseable upload resource name, missing 'uploads': %s", resourceName)
		}
		pieceIdx--
...
func MissingDigestError(d *repb.Digest) error {
	if d == nil {
		util.Infof("MissingDigestError called with nil digest. Stack trace:\n%s", string(debug.Stack()))
	}
	...
	if st, err := st.WithDetails(pf); err != nil {
		return statuserr.InternalErrorf("Digest %v not found.", d)
	} else {
		return st.Err()
	}
}
...
func ParseFunction(s string) (repb.DigestFunction_Value, error) {
	...
	return 0, statuserr.InvalidArgumentErrorf("unknown digest function %q", s)
}
