...
func (p *Publisher) Start(ctx context.Context) {
	go func() {
		...
		for r.Next() {
			...
			util.Infof("Retrying build event stream due to error: %s", err)
		}
		p.errCh <- err
	}()
}
...
func (p *Publisher) run(ctx context.Context) error {
	...
	go func() {
		...
		for {
			...
			doneReceiving <- statuserr.UnavailableErrorf("recv failed: %s", err)
			return
		}
	}()
	...
	for obe := range events {
		...
		if err := stream.Send(req); err != nil {
			return statuserr.UnavailableErrorf("send failed: %s", err)
		}
	}
	...
	if err := stream.CloseSend(); err != nil {
		return statuserr.UnavailableErrorf("close send failed: %s", err)
	}
...
func (p *Publisher) Finish() error {
	...
	if err := <-p.errCh; err != nil {
		return statuserr.UnavailableErrorf("failed to publish build event stream after multiple attempts: %s", err)
	}
	return nil
}
