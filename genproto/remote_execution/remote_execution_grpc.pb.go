...
type ExecutionClient interface {
	...
	// HACK(tylerw): Publish a stream of operation updates.
...
// UnimplementedActionCacheServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
...
// uncompressed form.
//
// Additionally, ByteStream requests MAY come with an additional plain text
// header that indicates the `resource_name` of the blob being sent.  The
// header, if present, MUST follow the following convention:
// * name: `build.bazel.remote.execution.v2.resource-name`.
// * contents: the plain text resource_name of the request message.
// If set, the contents of the header MUST match the `resource_name` of the
// request message.  Servers MAY use this header to assist in routing requests
...
// UnimplementedContentAddressableStorageServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
